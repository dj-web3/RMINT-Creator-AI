                    ...
                    {stepGroups.filter(g => g.assignedChef === chef && (!searchQuery || g.label.toLowerCase().includes(searchQuery.toLowerCase()))).map((g, i) => {
                      ...
                      return (
                        <motion.div
                          ...
                          onMouseMove={(e) => { if (hoveredGroup?.id === g.id) setTooltipPos({ x: e.clientX, y: e.clientY }); }}
                          onMouseLeave={() => { setHoveredGroup(null); }}
                          className="h-16 mx-1 rounded-xl border shadow-xl flex flex-col justify-center px-5 gap-0.5 hover:ring-4 hover:ring-slate-100 cursor-pointer"
                          style={{ backgroundColor: g.color || '#fff', borderColor: 'rgba(0,0,0,0.05)', width: `${widthPercent}%`, minWidth: '80px', position: 'relative', zIndex: hoveredGroup?.id === g.id ? 50 : 10 }}
                        >
      ...
      <AnimatePresence>
        {hoveredGroup && (
          <motion.div 
            ...
            style={{ position: 'fixed', left: tooltipPos.x + 16, top: tooltipPos.y - 200, pointerEvents: 'none' }}
