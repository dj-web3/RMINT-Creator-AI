...
const SidebarRight: React.FC<any> = ({ stepGroups, setStepGroups, nodes }) => {
  ...
  const updateGroup = (groupId: string, changes: Record<string, any>) => {
    setStepGroups((prev: any) => prev.map((g: any) => g.id === groupId ? { ...g, ...changes } : g));
  };
            ...
            <div key={group.id} className="bg-white rounded-3xl border border-slate-100 shadow-[0_10px_30px_rgba(0,0,0,0.03)] overflow-hidden transition-all hover:shadow-lg">
              <div className="p-4 bg-slate-900 text-white flex items-center justify-between gap-2">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-7 h-7 shrink-0 bg-orange-500 text-white rounded-xl flex items-center justify-center text-[10px] font-black shadow-lg shadow-orange-500/20">{idx + 1}</div>
                  <input
                    type="text"
                    value={group.label}
                    onClick={(e) => e.stopPropagation()}
                    onChange={(e) => updateGroup(group.id, { label: e.target.value.toUpperCase() })}
                    className="bg-transparent text-[10px] font-black uppercase tracking-tight text-white w-full focus:outline-none border-b border-transparent focus:border-white/30 transition-colors truncate"
                  />
                </div>
              </div>
              <div className="p-5">
                ...
                <div className="flex items-center gap-3 mb-5">
                  <div className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl px-3 py-2">
                    <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Start Time</p>
                    <input
                      type="text"
                      ...
                      className="bg-transparent text-[10px] font-black text-slate-800 w-full focus:outline-none"
                    />
                  </div>
                  <div className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl px-3 py-2">
                    <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1">Duration (min)</p>
                    <input
                      type="number"
                      min={1}
                      value={group.durationMinutes || ''}
                      onClick={(e) => e.stopPropagation()}
                      onChange={(e) => updateGroup(group.id, { durationMinutes: Number(e.target.value) })}
                      className="bg-transparent text-[10px] font-black text-slate-800 w-full focus:outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-50">
                  <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-2">Assign Chef</p>
                  <div className="flex flex-wrap gap-2">
                    {chefs.map(chef => (
                      <button
                        key={chef}
                        onClick={() => updateGroup(group.id, { assignedChef: chef })}
